...
public class CommonQueryDaoImp extends SimpleJdbcDaoBase implements CommonQueryDao {
    ...
    public List<ProveedorBean> getLstProveedores(String prazonSocial) {
        ...
        sql.append("SELECT TOP 51 A.* \n"
                + "FROM (\n"
                + "SELECT CPRO_RUC NU_RUC, CPRO_RAZSOC DESCRIPCION FROM IDOSGD.LG_PRO_PROVEEDOR \n");
        prazonSocial=BusquedaTextual.getContextValue(prazonSocial);
        if (prazonSocial != null && prazonSocial.trim().length() > 1) {
            sql.append(" WHERE CONTAINS(cpro_razsoc, :pBusquedaTextual) ");
            sqlContains = "SELECT RETORNO FROM [IDOSGD].PK_SGD_DESCRIPCION_QUERYFILTER('"+prazonSocial+"')";
        }
        sql.append(")A order by A.DESCRIPCION desc ");
        try {
    ...
    public List<DestinatarioOtroOrigenBean> getLstOtrosOrigenes(String pdescripcion){
        ...
        if (pdescripcion != null && pdescripcion.trim().length()>1) {
            sql.append(" WHERE CONTAINS(*, :pBusquedaTextual) ");
            sqlContains = "SELECT RETORNO FROM [IDOSGD].PK_SGD_DESCRIPCION_QUERYFILTER('"+pdescripcion+"')";
        }              
