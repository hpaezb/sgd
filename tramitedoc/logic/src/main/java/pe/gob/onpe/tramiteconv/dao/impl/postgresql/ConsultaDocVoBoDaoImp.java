...
//        sql.append("	(SELECT DE_EST FROM IDOSGD.TDTR_ESTADOS WHERE CO_EST || DE_TAB = DECODE(VB.IN_VB,'B','0',VB.IN_VB) || 'TDTV_PERSONAL_VB') DE_ES_DOC_EMI,");
        sql.append("	(SELECT DE_EST FROM IDOSGD.TDTR_ESTADOS WHERE CO_EST || DE_TAB = ");
//        sql.append(" DECODE(VB.IN_VB,'B','0',VB.IN_VB) ");
        sql.append("CASE\n"
                + "WHEN VB.IN_VB='B' THEN\n"
                + "'0' \n"
                + "ELSE VB.IN_VB \n"                
                + "END ");
        sql.append("|| 'TDTV_PERSONAL_VB') DE_ES_DOC_EMI,");
...
//                sql.append(" AND RE.FE_EMI between TO_DATE(:pFeEmiIni,'dd/mm/yyyy') AND TO_DATE(:pFeEmiFin,'dd/mm/yyyy') + 0.99999");
        ...
        if (pTipoBusqueda.equals("1")) {
            ...
            if (bDocVoBo.getAsunto()!= null && bDocVoBo.getAsunto().trim().length() > 1) {
                ...
                sql.append(" AND UPPER(RR.DE_ASU) LIKE '%'||:pDeAsunto||'%' ");
