...
public class RecepDocumentoAdmServiceImp implements RecepDocumentoAdmService{
    ...
    private void estadoRecepcionDocumento(BuscarDocumentoRecepBean buscarDocumentoRecepBean){
     ...
     }else if(sEstado.equals("02")){//Recibido
         buscarDocumentoRecepBean.setsEstadoDoc("1");
     }
    }
        ...
        try{
            ...
            //if(!sEsDocAnu.equals("0")){
                ...
                //td_pk_tramite.actualiza_estado(:b_04.nu_ann, :b_04.nu_emi, :b_04.nu_des, :b_04.es_doc_rec);
                    ...
                    String sInResPlaAte = documentoBean.getInResPlaAte();
                    String sInPlaAte = documentoBean.getInPlaAte();
                    String sFeAteDoc = documentoBean.getFeAteDoc();
                    String sFeArcDoc = documentoBean.getFeArcDoc();
                    System.out.println("documentoBean.getNuEmi()--> "+ documentoBean.getNuEmi());
                    System.out.println("documentoBean.getNuEmiDocOriPlazoAtencion()--> "+ documentoBean.getNuEmiDocOriPlazoAtencion());
                    System.out.println("documentoBean.getInResPlaAte()--> "+ documentoBean.getInResPlaAte());
                    System.out.println("documentoBean.getInPlaAte()--> "+ documentoBean.getInPlaAte());
                    System.out.println("documentoBean.getFeAteDoc()--> "+ sFeAteDoc);
                    System.out.println("documentoBean.getFeArcDoc()--> "+ sFeArcDoc);
                    if(sInResPlaAte != null && !sInResPlaAte.equals("")){
                        System.out.println("Ingresa 1--> ");
                        vReturn = recepDocumentoAdmDao.actualizarEstadoPlazoAtencion(documentoBean, "0");
                    }
//                    if(sInPlaAte != null && !sInPlaAte.equals("") && (documentoBean.getFeArcDoc() != null || documentoBean.getFeAteDoc() != null)){
//                        System.out.println("Ingresa 2--> ");
//                        vReturn = recepDocumentoAdmDao.actualizarEstadoPlazoAtencionSinDoc(documentoBean);
//                    }
                    if(sFeAteDoc!=null && !sFeAteDoc.equals("")){
                        System.out.println("Ingresa 2--> ");
                        if(sInPlaAte != null && !sInPlaAte.equals("")){
                            System.out.println("Ingresa 3--> ");
                            vReturn = recepDocumentoAdmDao.actualizarEstadoPlazoAtencionSinDoc(documentoBean);
                        }
                    }
                    if(sFeArcDoc!=null && !sFeArcDoc.equals("")){
                        System.out.println("Ingresa 4--> ");
                        if(sInPlaAte != null && !sInPlaAte.equals("")){
                            System.out.println("Ingresa 5--> ");
                            vReturn = recepDocumentoAdmDao.actualizarEstadoPlazoAtencionSinDoc(documentoBean);
                        }
                    }                    
                    /*[HPB-02/10/20] Inicio - Plazo de Atencion*/
    ...
    @Override
    public HashMap getDocumentosEnReferencia(BuscarDocumentoRecepBean buscarDocumentoRecepBean, String pnuPagina, int pnuRegistros){
        List<DocumentoBean> list = null;
        DocumentoBean documentoBean = null;
        ...
        try{
            ...
            if(documentoBean!=null && documentoBean.getMsjResult()!=null && documentoBean.getMsjResult().equals("OK")){
                ...
                list = recepDocumentoAdmDao.getDocumentosReferenciadoBusq(documentoBean,buscarDocumentoRecepBean.getsTiAcceso(), pnuPagina, pnuRegistros);                
...
//                    }
    ...
    public List<DocumentoBean> getDocumentosBuscaRecepAdm2(BuscarDocumentoRecepBean buscarDocumentoRecep, String pnuPagina, int pnuRegistros) {
        ...
        //estadoRecepcionDocumento(buscarDocumentoRecep);
        try{
            list = recepDocumentoAdmDao.getDocumentosBuscaRecepAdm2(buscarDocumentoRecep, pnuPagina, pnuRegistros);
        }catch(Exception ex){
            ex.printStackTrace();
        }
    ...
    public List<DocumentoBean> getDocumentosBuscaRecepAdm2Size(BuscarDocumentoRecepBean buscarDocumentoRecep) {
        List<DocumentoBean> list = null;
        try{
            list = recepDocumentoAdmDao.getDocumentosBuscaRecepAdm2Size(buscarDocumentoRecep);
        }catch(Exception ex){
            ex.printStackTrace();
        }
        return list;        
    ...
    @Override
    public HashMap getDocumentosEnReferenciaSize(BuscarDocumentoRecepBean buscarDocumentoRecepBean) {
        List<DocumentoBean> list = null;
        DocumentoBean documentoBean = null;
        HashMap mp = new HashMap();
        try{
            ...
            if(documentoBean!=null && documentoBean.getMsjResult()!=null && documentoBean.getMsjResult().equals("OK")){
                documentoBean.setCoDepDes(buscarDocumentoRecepBean.getsCoDependencia());
                documentoBean.setCoEmpDes(buscarDocumentoRecepBean.getsCoEmpleado());                
                list = recepDocumentoAdmDao.getDocumentosReferenciadoBusqSize(documentoBean,buscarDocumentoRecepBean.getsTiAcceso());                
                mp.put("recepDocumAdmList",list);
                mp.put("msjResult", "1");//Existe Documento
            }else{
                mp.put("msjResult", "0");//NO existe Documento
    ...
    public String actualizarEstadoPlazoAtencion(DocumentoBean documentoBean, String inButtonRed) {
        ...
        try{
            vResult = recepDocumentoAdmDao.actualizarEstadoPlazoAtencion(documentoBean, inButtonRed);
        }catch(Exception ex){
            vResult = "NO_OK";
            ex.printStackTrace();
        }
