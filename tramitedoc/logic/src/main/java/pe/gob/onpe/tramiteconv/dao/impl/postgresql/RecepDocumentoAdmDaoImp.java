...
//        sql.append(" '01', IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP(X.CO_EMP_DES),");
//        sql.append(" '02', IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP(X.NU_RUC_DES),");
//        sql.append(" '03', IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL(X.NU_DNI_DES),");
//        sql.append(" '04', IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN(X.CO_OTR_ORI_DES)");
//        sql.append(" ) DE_EMP_DES,");
        ...
        sql.append("NU_DES,COALESCE(B.CO_PRI,'1') CO_PRI, B.ES_DOC_REC,");
...
//        this.spPuActualizaGuiaMp = new SimpleJdbcCall(this.dataSource).withCatalogName("td_pk_tramite").withProcedureName("pu_actualiza_guia_mp")
          this.spPuActualizaGuiaMp = new SimpleJdbcCall(this.dataSource).withFunctionName("IDOSGD.PK_SGD_TRAMITE_pu_actualiza_guia_mp")
    ...
    public String actualizarEstado(DocumentoBean documentoBean) {
        ...
        this.spActualizaEstado = new SimpleJdbcCall(this.dataSource).withFunctionName("IDOSGD.PK_SGD_TRAMITE_actualiza_estado")
                .withoutProcedureColumnMetaDataAccess()
    ...
    public String updDocumentoBean(DocumentoBean documentoBean, String paccion) {
        ...
        try {
            ...
            this.jdbcTemplate.update(sql.toString(), new Object[]{
                ...
                documentoBean.getDeAne(),documentoBean.getNuAnn(), 
                documentoBean.getNuEmi(), Integer.parseInt(documentoBean.getNuDes())});
            vReturn = "OK";
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        sql.append("	   )");
//        sql.append("	)");
//        sql.append(" AND COALESCE(TI_EMI_REF,'0') = NVL(?/*:B_01_ANN.TI_EMI_REF*/, COALESCE(TI_EMI_REF,'0'))");
//        sql.append(" AND NVL(CO_EMP_DES,'NULO') = NVL(?/*:B_01_ANN.TI_EMP_DES*/, NVL(CO_EMP_DES,'NULO'))");
//        sql.append(" AND (  (   NVL(?/*:B_01_ANN.LI_EXPE*/, 'NULO') = 'SINEX'");
//        sql.append("	   AND CO_EXP IS NULL");
//        sql.append("	   )");
...
//        sql.append("				  '03', PK_SGD_DESCRIPCION_ANI_SIMIL (B.NU_DNI_DES),");
...
//        sql.append("				 ) DE_EMP_DES,");
        ...
        sql.append(" C.IN_EXISTE_DOC EXISTE_DOC,"
//                + "DECODE(NVL(C.TI_EMI_REF,'0')||NVL(C.IN_EXISTE_ANEXO,'2'),'00',0,'02',0,1) EXISTE_ANEXO,"
                +" CASE"
                + " WHEN COALESCE(C.TI_EMI_REF,'0')||COALESCE(C.IN_EXISTE_ANEXO,'2')='00' THEN 0 "
                + " WHEN COALESCE(C.TI_EMI_REF,'0')||COALESCE(C.IN_EXISTE_ANEXO,'2')='02' THEN 0 "
                + " ELSE 1 END AS EXISTE_ANEXO,"
