...
public class ConsultaRecepDocServiceImp implements ConsultaRecepDocService {
    ...
    @Override
    public HashMap getDocumentosEnReferencia(BuscarDocumentoRecepConsulBean buscarDocumentoRecepConsulBean, String pnuPagina, int pnuRegistros){ //gct 30/07/2020
    //public HashMap getDocumentosEnReferencia(BuscarDocumentoRecepConsulBean buscarDocumentoRecepConsulBean){
        ...
        try{
            ...
            if(documentoRecepConsulBean!=null && documentoRecepConsulBean.getMsjResult()!=null && documentoRecepConsulBean.getMsjResult().equals("OK")){
                ...
                //list = consultaRecepDocDao.getDocumentosReferenciadoBusq(documentoRecepConsulBean,buscarDocumentoRecepConsulBean.getTiAcceso(), pnuPagina, pnuRegistros); //gct 30/07/2020
    ...
    public List<DocumentoRecepConsulBean> getDocumentosBuscaRecepAdmFiltro(BuscarDocumentoRecepConsulBean buscarDocumentoRecepConsulBean, String pnuPagina, int pnuRegistros) {
        List<DocumentoRecepConsulBean> list = null;
        try{
            list = consultaRecepDocDao.getDocumentosBuscaRecepAdmFiltro(buscarDocumentoRecepConsulBean, pnuPagina, pnuRegistros);
        }catch(Exception ex){
            ex.printStackTrace();
        }
    ...
    public List<DocumentoRecepConsulBean> getDocumentosBuscaRecepAdmFiltroSize(BuscarDocumentoRecepConsulBean buscarDocumentoRecepConsulBean) {
        List<DocumentoRecepConsulBean> list = null;
        try {
            list = consultaRecepDocDao.getDocumentosBuscaRecepAdmFiltroSize(buscarDocumentoRecepConsulBean);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    //gct 30/07/2020
    @Override
    public HashMap getDocumentosEnReferenciaSize(BuscarDocumentoRecepConsulBean buscarDocumentoRecepConsulBean) {
        List<DocumentoRecepConsulBean> list = null;
        DocumentoRecepConsulBean documentoRecepConsulBean = null;
        HashMap mp = new HashMap();        
        ...
        try{
            ...
            if(documentoRecepConsulBean!=null && documentoRecepConsulBean.getMsjResult()!=null && documentoRecepConsulBean.getMsjResult().equals("OK")){
                documentoRecepConsulBean.setCoDepDes(buscarDocumentoRecepConsulBean.getCoDependencia());
                documentoRecepConsulBean.setCoEmpDes(buscarDocumentoRecepConsulBean.getCoEmpleado()); 
                //list = consultaRecepDocDao.getDocumentosReferenciadoBusqSize(documentoRecepConsulBean,buscarDocumentoRecepConsulBean.getTiAcceso());                
                mp.put("recepDocumAdmList",list);
                mp.put("msjResult", "1");//Existe Documento
            }else{
                mp.put("msjResult", "0");//NO existe Documento
