...
public class DocumentoObjDaoImp extends SimpleJdbcDaoBase implements DocumentoObjDao {
    ...
    public DocumentoDatoBean getDatosDoc(String pnuAnn, String pnuEmi){
        ...
        sql.append("select \n" +
                    ...
                    "INITCAP(PK_SGD_DESCRIPCION.DE_DISTRITO_LOCAL(co_loc_emi)) de_Lugar\n" +
                    "from tdtv_remitos\n" +
                    "where nu_ann = ?\n" +
                    "and nu_emi = ?");
    ...
    public List<DocumentoDatoBean> CargarSubReporte1(String pnuAnn, String pnuEmi) {
        StringBuilder sql = new StringBuilder();
            sql.append("select    a.nu_ann NU_ANN_R,\n" +
                        ...
                        "	END DOCUMENTO,\n" +
                        "	b.fe_emi FE_EMI_R,\n" +
                        "	LTRIM(RTRIM(SUBSTR(b.de_asu, 1, 200))) DE_ASU_R\n" +
    ...
    public List<DocumentoDatoBean> CargarSubReporte2(String pnuAnn, String pnuEmi) {
        StringBuilder sql = new StringBuilder();
            sql.append("SELECT  a.nu_ann,\n" +
                        ...
                        "		ELSE PK_SGD_DESCRIPCION.DE_DEPENDENCIA(b.co_dep_des) || '\n' || (\n" +
                        "				CASE b.ti_des\n" +
                        "					WHEN '01' THEN PK_SGD_DESCRIPCION.DE_NOM_EMP(b.co_emp_des)\n" +
                        "					WHEN '02' THEN PK_SGD_DESCRIPCION.DE_PROVEEDOR(B.NU_RUC_DES)\n" +
                        "					WHEN '03' THEN PK_SGD_DESCRIPCION.ANI_SIMIL(b.Nu_Dni_Des)\n" +
    ...
    public List<DocumentoObjBean> leerDocumentoAnexo(String pnuAnn, String pnuEmi) {
        List<DocumentoObjBean> list = new ArrayList<DocumentoObjBean>();
        ...
        sql.append("select \n" +
                    ...
                    "and nu_emi = ? ");
            try {
                list = this.jdbcTemplate.query(sql.toString(), new RowMapper<DocumentoObjBean>() {                    
                    public DocumentoObjBean mapRow(ResultSet rs, int i) throws SQLException {
                                
                            int size=0;
                            DocumentoObjBean documentoObjBean = new DocumentoObjBean();
                            try {
                                ...
                                if(documento!=null){
                                    ...
                                    */
                                    documentoObjBean.setNuAnn(rs.getString("nu_ann"));
                                    documentoObjBean.setNuEmi(rs.getString("nu_emi"));
                                    documentoObjBean.setNombreArchivo(rs.getString("de_ruta_origen"));
                                    ...
                                    try {
                                        ...
                                        baos.flush();
                                        documentoObjBean.setDocumento(baos.toByteArray());
                                    }finally {
                                         if (blobStream != null) {
                                              blobStream.close();
                                         }
                                ...
                                StringBuilder mensaje = new StringBuilder();
                                mensaje.append("LeerBlob:"+documentoObjBean.getNuAnn()+"."+documentoObjBean.getNuEmi());
                                logger.error(mensaje,e);            
                                //e.printStackTrace();
                            }
                            return documentoObjBean;
                    }                        
                }     
            ...
            /*
            new RowMapper<BRepInvAnosExp>() {
				@Override
				public BRepInvAnosExp mapRow(ResultSet rs, int arg1) throws SQLException {
					BRepInvAnosExp item = new BRepInvAnosExp("", rs.getString(1), rs.getString(2), "");
					cantidadTotal[idxCT]+=rs.getInt(2);
					return item;
				}
			}
            */
