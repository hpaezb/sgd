...
public class DocumentoMensajeriaServiceImp implements DocumentoMensajeriaService {
    ...
    public List<TipoElementoMensajeriaBean> getListResponsableMensajeria(String tipo,String Ambito){
     List<TipoElementoMensajeriaBean> list = null;
        try {  
            list = mensajeriaDao.getListResponsableMensajeria(tipo,Ambito);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public String insMensajeriaDocumento(DocumentoRecepMensajeriaBean documentoMensajeria){
      ...
      @Override
    public String updMensajeriaDocumentoRecibir(String codigos, String usuario){
        String vReturn = "ERR";
        try{
           vReturn =   mensajeriaDao.updMensajeriaDocumentoRecibir(codigos, usuario);/*[HPB-21/06/21]Fin Campos Auditoria-*/
        }catch(Exception e){
            vReturn = "ERR";
        }
     ...
     @Override
    public String updMensajeriaDocumentoDevolver(String codigos, String pEmiInsEstacionDoc, DocumentoEmiBean documentoEmiBean){
        String vReturn = "ERR";
        try{
           vReturn =   mensajeriaDao.updMensajeriaDocumentoDevolver(codigos, pEmiInsEstacionDoc, documentoEmiBean);
        }catch(Exception e){
            vReturn = "ERR";
        }
    ...
    @Override
    public ReporteBean getGenerarReporte(BuscarDocumentoRecepMensajeriaBean buscarDocumentoRecepMensajeriaBean, Map parametros) {
        List lista = null;
        ReporteBean objReporte = new ReporteBean();
        String coRespuesta="";
        String deRespuesta="";
        ...
        try {
            ...
            try{
                lista = getDocumentoRecepMensajeria(buscarDocumentoRecepMensajeriaBean);
                eserror="0";
            }catch(Exception ex){               
                eserror="1";
                deRespuesta=ex.getMessage();
            ...
            if(buscarDocumentoRecepMensajeriaBean.getFormatoReporte().equalsIgnoreCase("PDF")){
                coReporte = "TDR_MSJ";                
                extensionArch=".pdf";
                tipoArchivo = 2;
            }else{
                coReporte = "TDR_GESDOC_XLS";
                extensionArch=".xls";
                tipoArchivo = 1;
            }
            
            String rutaJasper =  buscarDocumentoRecepMensajeriaBean.getRutaReporteJasper() + "/" + coReporte + ".jasper";
            buscarDocumentoRecepMensajeriaBean.setRutaReporteJasper(rutaJasper);
            ...
            Utilidades util = new Utilidades();
            archivo = util.GenerarReporteLista(buscarDocumentoRecepMensajeriaBean.getRutaReporteJasper(), lista, coReporte, parametros, tipoArchivo);
    ...
    public List<DestiDocumentoEnvMensajeriaBean> getLstDestinoEnvMensajeria2(String pnuAnnpnuEmi, DocumentoRecepMensajeriaBean documentoRecepMensajeriaBean) {
        List<DestiDocumentoEnvMensajeriaBean> list = null;
        try {  
            list = mensajeriaDao.getLstDestinoEnvMensajeria2(pnuAnnpnuEmi, documentoRecepMensajeriaBean);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public String insMensajeriaDocumento2(DocumentoRecepMensajeriaBean documentoMensajeria, List<DestiDocumentoEnvMensajeriaBean> lstDestinos) {
        String vReturn = "ERR";
        try{
           vReturn =   mensajeriaDao.insMensajeriaDocumento2(documentoMensajeria, lstDestinos);
        }catch(Exception e){
            vReturn = "ERR";
        }
    ...
    public List<DocumentoRecepMensajeriaBean> getLstDetalleMensajeriaVirtual(DestinoResBen oDestinoResBen) {
        List<DocumentoRecepMensajeriaBean> list = null;
        try {  
            list = mensajeriaDao.getLstDetalleMensajeriaVirtual(oDestinoResBen);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return list;        
