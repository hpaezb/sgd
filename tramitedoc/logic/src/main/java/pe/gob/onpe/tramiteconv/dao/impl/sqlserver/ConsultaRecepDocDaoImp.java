                ...
                if(vFeEmiIni!= null && vFeEmiIni.trim().length()>0 &&
                   vFeEmiFin!= null && vFeEmiFin.trim().length()>0){
                    if(bBusqFeEmi){
                        ...
                        sql.append(" AND CAST(B.FE_REC_DOC AS DATE) BETWEEN CAST(:pFeEmiIni AS DATE) AND CAST(:pFeEmiFin AS DATE) ");
                    }
                       ...
                       vFeEmiFin!= null && vFeEmiFin.trim().length()>0){
                        if(bBusqFeEmi){
                           ...
                           prutaReporte.append(" AND CAST(A.FE_EMI AS DATE) BETWEEN CAST(:pFeEmiIni AS DATE) AND CAST(:pFeEmiFin AS DATE) ");
                            ...
                            //prutaReporte.append(" AND CAST(B.FE_REC_DOC AS DATE) between CAST('").append(vFeEmiIni).append("' AS DATE)").append(" AND CAST('").append(vFeEmiFin)
                            //            .append("' AS DATE) ");
                            prutaReporte.append(" AND CAST(B.FE_REC_DOC AS DATE) BETWEEN CAST(:pFeEmiIni AS DATE) AND CAST(:pFeEmiFin AS DATE) ");
                        }                        
                        objectParam.put("pFeEmiIni", vFeEmiIni);
                        objectParam.put("pFeEmiFin", vFeEmiFin); 
                    }
