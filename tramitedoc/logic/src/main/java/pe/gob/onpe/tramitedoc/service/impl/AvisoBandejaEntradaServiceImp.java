...
public class AvisoBandejaEntradaServiceImp implements AvisoBandejaEntradaService {
    ...
    //@Override
    @Override
    public List<AvisoBandejaEntradaBean> getBandejaEntrada(String coDependencia, String coEmpleado, String tipoAcesso) {
        List<AvisoBandejaEntradaBean> list = null;
        try {
            if (tipoAcesso.equals("0")) {
                list = avisoBandejaEntradaDao.getBandejaEntradaAccesoTotal(coDependencia,coEmpleado);
            } else if (tipoAcesso.equals("2")) {
                list = avisoBandejaEntradaDao.getBandejaEntradaAccesoFuncionario(coDependencia, coEmpleado);
            } else {
                list = avisoBandejaEntradaDao.getBandejaEntradaAccesoPersonal(coDependencia, coEmpleado);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
