...
public class ConsultaEmiDocDaoImp extends SimpleJdbcDaoQuery implements ConsultaEmiDocDao{
    ...
    private static Logger logger=Logger.getLogger(ConsultaEmiDocDaoImp.class);
    ...
    public DocumentoEmiConsulBean getDocumentoEmiAdm(String pnuAnn, String pnuEmi) {
        ...
        sql.append("SELECT TO_CHAR((SELECT FE_EXP FROM TDTC_EXPEDIENTE WHERE NU_ANN_EXP=A.NU_ANN_EXP AND NU_SEC_EXP=A.NU_SEC_EXP),'DD/MM/YYYY') FE_EXP_CORTA,\n" +
                    ...
                    "PK_SGD_DESCRIPCION.DE_PROCESO_EXP(B.CO_PROCESO_EXP) DE_PROCESO_EXP,PK_SGD_DESCRIPCION.ESTADOS(A.ES_DOC_EMI,'TDTV_REMITOS') DE_ES_DOC_EMI, A.ES_DOC_EMI ES_DOC_EMI, \n" +
    ...
    public List<DocumentoEmiConsulBean> getDocumentosReferenciadoBusq(DocumentoEmiConsulBean documentoEmiConsulBean,String sTipoAcceso, String pnuPagina, int pnuRegistros){       
        ...
        Map<String, Object> objectParam = new HashMap<String, Object>();
        List<DocumentoEmiConsulBean> list = new ArrayList<DocumentoEmiConsulBean>();        
        sql.append("SELECT * FROM (SELECT A.*, ROWNUM row_number FROM ( ");/* HPB 28/05/2020 - Requerimiento Paginación consulta emitidos */
...
*/
    ...
    public List<DocumentoEmiConsulBean> getListaReporteBusqueda(BuscarDocumentoEmiConsulBean buscarDocumentoEmiConsulBean) {
        ...
        try {
            ...
            logger.info("SQL Reporte: "+prutaReporte);
            System.out.println("SQL Reporte : "+prutaReporte);
            list = this.namedParameterJdbcTemplate.query(prutaReporte.toString(), objectParam, BeanPropertyRowMapper.newInstance(DocumentoEmiConsulBean.class));
    ...
    public List<DocumentoEmiConsulBean> getDocumentosBuscaEmiAdmFiltro(BuscarDocumentoEmiConsulBean buscarDocumentoEmiConsulBean, String pnuPagina, int pnuRegistros) {
        ...
        sql.append(" A.ES_DOC_EMI,A.NU_DIA_ATE,A.CO_TIP_DOC_ADM,A.NU_CANDES,A.CO_EMP_RES, ");        
        sql.append(" CASE A.TI_ENV_MSJ WHEN '0' THEN 'MESA DE PARTES' WHEN '1'  THEN ( CASE WHEN NVL(A.NENVIOCORREO,'00')='01' THEN 'ENVÍO CORREO' ELSE  'ENVÍO DIRECTO' END )  END  recursoenvio ");        
        //sql.append(" FROM TDTV_REMITOS A,TDTX_REMITOS_RESUMEN B, TDTV_DESTINOS C"); /*HPB 18/09/2019 Se comenta por duplicidad en registros*/
        sql.append(" FROM TDTV_REMITOS A,TDTX_REMITOS_RESUMEN B");
        sql.append(" WHERE");
        sql.append(" B.NU_ANN=A.NU_ANN");
        sql.append(" AND B.NU_EMI=A.NU_EMI");        
        //sql.append(" AND C.NU_ANN = B.NU_ANN");/*HPB 18/09/2019 Se comenta por duplicidad en registros*/
        //sql.append(" AND C.NU_EMI = B.NU_EMI");/*HPB 18/09/2019 Se comenta por duplicidad en registros*/
        ...
        if (pTipoBusqueda.equals("0") || bBusqFiltro) {
            ...
            if (buscarDocumentoEmiConsulBean.getCoDepDestino() != null && buscarDocumentoEmiConsulBean.getCoDepDestino().trim().length() > 0) {
                if(buscarDocumentoEmiConsulBean.getTiDes().equals("01") || buscarDocumentoEmiConsulBean.getTiDes()=="01"){
                    sql.append(" AND INSTR(B.TI_EMI_DES, :pTiEmpPro) > 0 ");
                    objectParam.put("pTiEmpPro", buscarDocumentoEmiConsulBean.getCoDepDestino());
                }else{
                    sql.append(" AND C.TI_DES = :pTiDes ");
                    objectParam.put("pTiDes", buscarDocumentoEmiConsulBean.getTiDes());
                }
            }
            if (buscarDocumentoEmiConsulBean.getEsFiltroFecha() != null
                    && (buscarDocumentoEmiConsulBean.getEsFiltroFecha().equals("1") || buscarDocumentoEmiConsulBean.getEsFiltroFecha().equals("3"))) {
        ...
        logger.info("SQL: Consulta Documentos Emitidos: "+ sql);
        System.out.println("SQL CONSULTA DOCUMENTOS EMITIDOS-->"+sql);
        /*System.out.println("pnuPagina-->"+pnuPagina);
        System.out.println("pnuRegistros-->"+pnuRegistros);        
        */
        try {
            list = this.namedParameterJdbcTemplate.query(sql.toString(), objectParam, BeanPropertyRowMapper.newInstance(DocumentoEmiConsulBean.class));
