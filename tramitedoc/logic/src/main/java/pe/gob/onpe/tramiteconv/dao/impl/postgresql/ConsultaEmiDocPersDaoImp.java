...
//        sql.append(" DECODE(X.NU_CANDES, 1, TD_PK_TRAMITE.TI_DES_EMP(X.NU_ANN, X.NU_EMI),TD_PK_TRAMITE.TI_DES_EMP_V(X.NU_ANN, X.NU_EMI)) DE_DEP_DESTINO,");
        sql.append("CASE\n" +
...
//                    "NVL2(a.co_loc_des,substr(TD_PK_DESCRIPCION.DE_LOCAL(a.co_loc_des), 1, 100),NULL) de_local,\n" +
...
//                    "NVL2(a.co_dep_des,substr(TD_PK_DESCRIPCION.DE_DEPENDENCIA(a.co_dep_des), 1, 100),NULL) de_dependencia,\n" +
...
//                    "NVL2(a.co_emp_des,substr(TD_PK_DESCRIPCION.DE_NOM_EMP(a.co_emp_des), 1, 100),NULL) de_empleado,\n" +
                    ...
                    "a.co_mot co_tramite," +
//                    "NVL2(a.co_mot,substr(TD_PK_DESCRIPCION.MOTIVO(a.co_mot), 1, 100),NULL) de_tramite,\n" +
                ...
                if (buscarDocPer.getTipoDoc()!= null && buscarDocPer.getTipoDoc().trim().length()>0){
                 ...
                 if(buscarDocPer.getEsFiltroFecha()!= null && 
                    (buscarDocPer.getEsFiltroFecha().equals("1") || buscarDocPer.getEsFiltroFecha().equals("3"))){
                     ...
                     if(vFeEmiIni!= null && vFeEmiIni.trim().length()>0 &&
                        vFeEmiFin!= null && vFeEmiFin.trim().length()>0){                    
                         //sql.append(" AND CAST(R.FE_EMI AS DATE) BETWEEN CAST('").append(vFeEmiIni).append("' AS DATE)").append(" AND CAST('").append(vFeEmiFin)
                         //   .append("' AS DATE) ");                    
                         sql.append(" AND R.FE_EMI between to_timestamp(:pFeEmiIni,'dd/mm/yyyy') AND to_timestamp(:pFeEmiFin,'dd/mm/yyyy hh24:mi:ss')");
                         objectParam.put("pFeEmiIni", vFeEmiIni);
