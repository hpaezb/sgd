    ...
    $("#myTableFixed tbody tr").click(function(e) {
        if ($(this).hasClass('row_selected')) {
            ...
            if (typeof($(this).children('td')[12]) !== "undefined") {
                ...
                jQuery('#txtCoEstadoDoc').val($(this).children('td')[19].innerHTML);//Hermes - 28/05/2019
...
function submitAjaxFormRecepDocAdmConsul(tipo){
    ...
    if (validaFiltro==="1") {
        ajaxCall("/srConsultaRecepcionDoc.do?accion=goInicio&vPagina="+paginaRecepcionSelect,$('#buscarDocumentoRecepConsulBean').serialize(),function(data){
                refreshScript("divTblConsulDocumentoRecepcionado", data);
        }, 'text', false, false, "POST");        
    }else if(validaFiltro === "2"){//buscar referencia
        ajaxCall("/srConsultaRecepcionDoc.do?accion=goBuscaDocumentoEnReferencia&vPagina="+paginaRecepcionSelect, $('#buscarDocumentoRecepConsulBean').serialize(), function(data) {
...
/*GCT 31/07/2020 - Inicio - Requerimiento Paginación consulta recepcion*/
function fn_paginationChangeTipoBusqConsulDocuAdmNext2(){
    ...
    fn_changePaginaConsulDocuAdm2()
}

function fn_paginationChangeTipoBusqConsulDocuAdmNextLast2(){
    var ultimaPagina = $('select option:last').val();    
    $('select[name=cantPaginasRecep]').val(ultimaPagina);
    fn_changePaginaConsulDocuAdm2()
}

function fn_paginationChangeTipoBusqConsulDocuAdmPrevius2(){
    var paginaInicial= $('select[name=cantPaginasRecep]').val();
    var paginaNext = parseInt(paginaInicial) - 1;
...
function fn_paginationChangeTipoBusqConsulDocuAdmPreviusFirts2(){
    var PrimeraPagina = $('select option:first').val();
    $('select[name=cantPaginasRecep]').val(PrimeraPagina);
    fn_changePaginaConsulDocuAdm2()
}

function fn_changePaginaConsulDocuAdm2(){   
  jQuery('#txtPaginaRecepcion').val($('select[name=cantPaginasRecep]').val());
  var tipo = jQuery('#buscarDocumentoRecepConsulBean').find('#tipoBusqueda').val();
  changeTipoBusqRecepDocuAdmConsul(tipo);
}
/*GCT 31/07/2020 - Fin - Requerimiento Paginación consulta recepcion*/