...
function fn_fileUpload(reemplazarDoc, tamanioMaxAnexos) {
    $("#progress").show();
    //var vFileSizeMax = $("#txtLimiteUpdAnexo").val();Hermes 28/06/19
...
//    var rutactx = $("#rutactx").attr("value");
    ...
    jQuery('#fileupload').fileupload({
        ...
        add: function(e, data) {
            ...
            /*Inicio-------------Valida PDF----------------------*/            
            var extensiones_permitidas = new Array(".doc",".docx", ".xls", ".xlsx", ".pdf", ".png", ".jpeg", ".jpg");
            ...
            if (fileSize <= fileSizeMax)
            {
                ...
                if (reemplazarDoc === true) {
                    ...
                    var url = "";
                    console.log("nuAnn="+nuAnn);
                    console.log("nuEmi="+nuEmi);
            ...
            if (data.textStatus === "success")
            {
                ...
                if (contAux === numFiles) {
                    if (reemplazarDoc) {
                        ...
                        $("#Anexo" + nuAne + " input[name=reqPublic]").prop('checked', true);
...
{
    var pkEmiDocActual = jQuery("#Actual_pkEmiDoc").attr("value");
    var vPublico = jQuery("#txtPublico").attr("value");
    if (pkEmiDocActual) {
        fn_verListadoArchivosAnexos(pkEmiDocActual, vPublico);
    }
}


function fn_verListadoArchivosAnexos(vpkEmiDoc, vPublico) {
    console.log("fn_verListadoArchivosAnexos");
    var p = new Array();
    var accion;
    if (esElNodoPadre()) {
...
function fn_cargarAnexos(tamanioMaxAnexos) {
    jQuery('#fileupload').attr("multiple", "");
    jQuery('#progress .progress-bar').css('width', '0%');
    jQuery('#progress span').html('');
    fn_fileUpload(false, tamanioMaxAnexos);/*HPB 17/02/2020 - Requerimiento capacidad archivo configurable*/
}
...
{
    console.log("fn_GuardarCambios()");
    var cambioDetalle = false;
    var indiceCambios = Array();
    var jsonBody = [];
    var nuAnn = $("#oNuAnn").attr("value");
    var nuEmi = $("#oNuEmi").attr("value");
    var pesProyecto = $("#oEsProyecto").attr("value");
    
    jQuery("#txtProyectoDocumento").val($('input[name=esProyecto]').prop('checked')); /*-----Hermes 29/04/19 -----*/   
    
    console.log("esProyecto ="+pesProyecto);
    console.log("nuAnn ="+nuAnn);
    console.log("nuEmi ="+nuEmi);
    ...
    jQuery("[name='txtDetalleAnexo']").each(function(indice, valor) {
        ...
        var esProyecto=1*jQuery(jQuery(jQuery(this).parents('tr')).find('[name=esProyecto]')).is(':checked');
        var esPublico=1*jQuery(jQuery(jQuery(this).parents('tr')).find('[name=reqPublic]')).is(':checked');
        console.log("entro ="+indice);
        console.log("firma->"+reqFirma);
        console.log("proyecto->"+esProyecto);
        
        if (typeof (esProyecto) !== "undefined" && esProyecto === 1) {
            contproyecto++;
        }
        
        if ([jQuery(this).val(),reqFirma,esProyecto,esPublico].join() !== arrayDetalles[indice].join())
        {
            console.log("entro if reqFirma o proyecto");
            cambioDetalle = true;
            ...
            jsonBody.push({"nuAnn": nuAnn, "nuEmi": nuEmi, "nuAne": nuAne, "deDet": nuevoDetalle, "tiOpe": "update","reqFirma": reqFirma,"esProyecto": esProyecto, "esPublico": esPublico});
        }
        
        console.log("sali principal");
    });
    
    console.log("contproyecto->"+contproyecto);
    if(contproyecto>1){
        alert_Warning("MENSAJE", "No puede tener más de un proyecto.");
    }else{
...
function fn_eventDocAnexos() {
    ...
    $("#tblDocAnexos tbody tr").click(function() {
        ...
        //console.log(indexFilaClick);
        if (indexFilaClick !== -1) {
            var btn_doc_sub = $("#tblDocAnexos tbody tr:eq(" + indexFilaClick + ") div").find('#btn-doc-sub');
            var isBtn_doc_sub=!!btn_doc_sub.length;
            console.log('isBtn_doc_sub-->'+isBtn_doc_sub);
            if ($("#tblDocAnexos tbody tr:eq(" + indexFilaClick + ")").hasClass('row_selected')) {
                ...
                if(isBtn_doc_sub){
                    $("#tblDocAnexos tbody tr:eq(" + indexFilaClick + ") div button:gt(1)").remove();
                }else{
        ...
        if ($(this).hasClass('row_selected')) {
            ...
            //console.log("indexFilaClick->"+indexFilaClick);
    ...
    var vEsProyecto = $("#oEsProyecto").attr("value");
    console.log("vEsProyecto->"+vEsProyecto);
    if (typeof (vEsProyecto) !== "undefined" && vEsProyecto !== "1") {
        $('#tblDocAnexos tbody tr>td:nth-child(4)').click(function(e) {
            e.stopPropagation();
        });
    }else{
        $('#tblDocAnexos tbody tr>td:nth-child(5)').click(function(e) {
            e.stopPropagation();
        });
    }
...
function fn_rptaCargarDocAnexoFirmado(data) {
    if (data !== null) {
        if (data.coRespuesta === "1") {
            /*-----[HPB] 12/02/21 Requiere firma anexo----*/
            var nuAne = $("#oNuAne").attr("value");
            var indice = $("tr [nuAne='" + nuAne + "']").parents("tr").index();
            jQuery("#tblDocAnexos tbody tr:eq(" + indice + ")").find('[name=reqFirma]').prop('checked', false);
            /*-----[HPB] 12/02/21 Requiere firma anexo-----*/
