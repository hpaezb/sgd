...
function submitAjaxFormEmiDocAdmConsul(tipo) {
    ...
    if (validaFiltro === "1") {
        ajaxCall("/srConsultaEmisionDoc.do?accion=goInicio&vPagina="+paginaRecepcionSelect, $('#buscarDocumentoEmiConsulBean').serialize(), function(data) {
            refreshScript("divTblConsulDocumentoEmitido", data);
        }, 'text', false, false, "POST");
    }else if(validaFiltro === "2"){//buscar referencia
        ajaxCall("/srConsultaEmisionDoc.do?accion=goBuscaDocumentoEnReferencia&vPagina="+paginaRecepcionSelect, $('#buscarDocumentoEmiConsulBean').serialize(), function(data) {
...
function fu_setDatoDestinatarioEmiConsul(cod, desc, tides) {
    ...
    jQuery('#buscarDocumentoEmiConsulBean').find('#tiDes').val(tides);//Hermes 05/09/2018
    removeDomId('windowConsultaDestinoEmi');
    ...
    $("#myTableFixed tbody tr").click(function(e) {
        if ($(this).hasClass('row_selected')) {
            ...
            if (typeof($(this).children('td')[14]) !== "undefined") {
                jQuery('#txtpnuAnn').val($(this).children('td')[14].innerHTML);
                jQuery('#txtpnuEmi').val($(this).children('td')[15].innerHTML);
                jQuery('#txtCoEstadoDoc').val($(this).children('td')[20].innerHTML);//Hermes - 28/05/2019
...
function fn_iniConsDestEmiConsul(){
        ...
        var searchOnTable = function(evento) {
                ...
                if(evento.which == 13){
                    if(isFirst){
                        ...
                        var ptiDes= $("#tlbDestinoEmi tbody tr:eq("+indexSelect+")").find("td:eq(3)").html();//Hermes 05/09/2018
                        fu_setDatoDestinatarioEmiConsul(pcodDest,pdesDest,ptiDes);//Hermes 05/09/2018
                    }
                }
        };        
        ...
        $("#tlbDestinoEmi tbody tr").click(function(e) {
            ...
            var ptiDes= $(this).find("td:eq(3)").html();//Hermes 05/09/2018
            fu_setDatoDestinatarioEmiConsul(pcodDest,pdesDest,ptiDes);//Hermes 05/09/2018            
        });    
}
...
/*HPB 28/05/2020 - Inicio - Requerimiento Paginación consulta emitidos*/
function fn_paginationChangeTipoBusqConsulDocuAdmNext(){
    ...
    fn_changePaginaConsulDocuAdm()
}

function fn_paginationChangeTipoBusqConsulDocuAdmNextLast(){
    var ultimaPagina = $('select option:last').val();    
    $('select[name=cantPaginasRecep]').val(ultimaPagina);
    fn_changePaginaConsulDocuAdm()
}

function fn_paginationChangeTipoBusqConsulDocuAdmPrevius(){
    var paginaInicial= $('select[name=cantPaginasRecep]').val();
    var paginaNext = parseInt(paginaInicial) - 1;
...
function fn_paginationChangeTipoBusqConsulDocuAdmPreviusFirts(){
    var PrimeraPagina = $('select option:first').val();
    $('select[name=cantPaginasRecep]').val(PrimeraPagina);
    fn_changePaginaConsulDocuAdm()
}

function fn_changePaginaConsulDocuAdm(){   
  jQuery('#txtPaginaRecepcion').val($('select[name=cantPaginasRecep]').val());
  var tipo = jQuery('#buscarDocumentoEmiConsulBean').find('#tipoBusqueda').val();
  changeTipoBusqEmiDocuAdmConsul(tipo);
}
/*HPB 28/05/2020 - Fin - Requerimiento Paginación consulta emitidos*/