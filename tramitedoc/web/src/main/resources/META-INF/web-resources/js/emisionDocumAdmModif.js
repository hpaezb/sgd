...
function fn_verRegistrarAvance(){ 
    ...
    if(pnuAnn != null && pnuAnn!=""  ){ 
        if(!$.trim($("#txtAvanceDescripcion").val())){/*HPB 18/09/2019 Avance de recepcionados*/
            bootbox.alert('Debe ingresar alguna observación para registrar el avance del documento.');
        }else{
           var p = new Array();
...
//jazanero
function fn_buscarRemitenteCiudadanoEmisionPrevio(cell){
    ...
    //if(!!pnomOtro){
        ...
        if (rptValida === "OK") {
            ...
            if(pnomOtro.match(re)){*/
                ...
                var p = new Array();
                p[0] = "accion=goBuscaDestCiudadanoPrevio";      
                ajaxCall("/srDocumentoAdmEmision.do", p.join("&"), function(data) {
                    ...
                    fn_rptaBuscarRemitenteOtroDocExtRec(data);
                     ...
                     console.log("1->"+(($(cell).parent()).parent()).index());
                     console.log("2->"+($(cell).parent()).index());
                     console.log("3->"+(((($(cell).parent()).parent()).parent()).parent()).attr("id"));
...
function changeTipoEmision_cell(cell) {     
     ...
     if($(cell).val()=="03"){
        ...
        $("#"+tablaId+" tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('input[id=txtNombresPJCiudadano]').attr('readonly','readonly'); 
    ...
    if($(cell).val()=="04"){
        ...
        $("#"+tablaId+" tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('input[id=txtNombresPJCiudadano]').removeAttr('readonly');
...
function fn_buscarRemitenteCiudadanoEmisionPrevioProy(cell){
     
    console.log("cell->"+cell);                    
    var p = new Array();
    p[0] = "accion=goBuscaDestCiudadanoPrevioProy";      
    ajaxCall("/srDocumentoAdmEmision.do", p.join("&"), function(data) {
...
function fn_iniConsCiudadanoDestEmiProy(){
    var nomTbl='#tblDestinatario';
    var indexAux=-1;
    var tableaux = $(nomTbl);
    tableaux.find('tr').each(function(index, row) {
    ...
    var searchOnTable = function(evento) {
            ...
            if(evento.which == 13){
                if(isFirst){
                 ...
                 fn_setDestinoEmitlbCiudadanoProy(nombres,dni);             
                }
            }else if(evento.which==38||evento.which==40){
                //$('#txtConsultaFind').blur();
                indexAux=-1;
...
function fn_setDestinoEmitlbCiudadanoProy(nombres, dni) {
    var pfila = jQuery('#txtTblDestProveedorEmiFilaWhereButton').val();
    var pcol = jQuery('#txtTblDestEmiProveedorColWhereButton').val(); 
    
    //var bResult = fn_validaDestinatarioOtroDuplicado('tblDestEmiDocAdmOtro', arrCampo);
    //if (bResult) { 
        $("#tblDestEmiDocAdmPersJuriProy tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('#txtDniPJCiudadano').val(dni);  
        $("#tblDestEmiDocAdmPersJuriProy tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('#txtCodigoRemitente').val(dni);    
        $("#tblDestEmiDocAdmPersJuriProy tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('#txtNombresPJCiudadano').val(nombres);         
