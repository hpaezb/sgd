function fn_iniDocMensajeria(){
    ...
    $('#buscarDocumentoRecepMensajeriaBean').find('#esFiltroFechaEnvMsj').val("3");//hoy
    $('#buscarDocumentoRecepMensajeriaBean').find("#fechaFiltroEnvMsj").showDatePicker({defaultOpcionSelected: 4});
    pnumFilaSelect=0;
...
function fu_cleanBusDocMensajeria(tipo) {
    ...
    if (tipo==="1") {
        ...
        /*----------------Hermes 15/08/2018-------------*/
        $(noForm).find("#tipoZona").val(".: TODOS :.");
...
function fu_iniTblDocMensajeriaRecep(){
    ...
    $("#myTableFixed tbody td").hover(
            function() {
                ...
                if ((index >= 3 && index <= 16 )|| index === 17 || index === 18  ) {
    ...
    $("#myTableFixed tbody tr").click(function(e) {
        if ($(this).hasClass('row_selected')) {
            ...
            if (typeof($(this).children('td')[0]) !== "undefined") {
                ...
                jQuery('#txtCoEstadoDoc').val($(this).children('td')[25].innerHTML);//Hermes - 28/05/2019
...
function fn_verAnexoMensajeriaRecep(){
    ...
    var pOpcion = allTrim($("#txtCodOpcion").val());//Hermes 28/05/2019
    if(!!pnuAnn&&!!pnuEmi){    
        fn_verAnexosObj(pnuAnn, pnuEmi, pnuDes, pOpcion);	
    }
}

//By César Mantilla
function fn_verDocMensajeriagm(){
    var pnuAnn=$('#txtpnuAnn').val();
    var pnuEmi=$('#txtpnuEmi').val();
    var ptiOpe = "0";
    if(!!pnuAnn&&!!pnuEmi){
...
function fn_verAnexoMensajeriaRecepAnex(){
    var pnuAnn=$('#txtpnuAnn').val();
    var pnuEmi=$('#txtpnuEmi').val();
    var pnuDes = "N";
    if(!!pnuAnn&&!!pnuEmi){    
...
function fn_verSeguimientoMensajeriaRecepAnex(){
    var pnuAnn=$('#txtpnuAnn').val();
    var pnuEmi=$('#txtpnuEmi').val();
    var pnuDes = "N";
    if(!!pnuAnn&&!!pnuEmi){    
...
//jazanero
function fu_generarReporteMensajeriaRecepXLS(){
    fu_generarReporteMsjRecep('XLS');
}
//jazanero
function fu_generarReporteMsjRecep(pformatoReporte){
    
    var validaFiltro = fu_validaFormBusDocMensajeria("0");

    if (validaFiltro === "1") {
        
        ajaxCall("/srMensajeriaGestion.do?accion=goExportarArchivoLista&pformatRepor="+pformatoReporte, $('#buscarDocumentoRecepMensajeriaBean').serialize(), function(data) {
            if (data!==null) {
...
function fn_optionSelecction(){
    var codigo="";
    var validar="";
    var feEnvOfi="";/*--21/08/19 HPB Devolucion Doc a Oficina--*/
    $('.esOptionCheck input').each(function() {  
        if($(this).is(':checked')){
            codigo+=$(this).attr("nuAnn")+$(this).attr("nuEmi")+",";
            validar+=$(this).attr("docEstadoMsj");
            feEnvOfi+=$(this).attr("feEnvOfi")+",";;/*--21/08/19 HPB Devolucion Doc a Oficina--*/
        }
    });
    $('#txtSelecctionOption').val(codigo);
    $('#txtSelecctionOptionValidar').val(validar);
    $('#txtSelecctionOptionFeEnvOfi').val(feEnvOfi); /*--21/08/19 HPB Devolucion Doc a Oficina--*/   
}
...
function fn_optionSelecctionDetalleMensajDest(){
    ...
    var otrosDatos="";
    var ambito="";
    var coTipoAccesoZona = "";//Hermes 19/09/2018
    var coTipoAccesoZonaNew = "";//Hermes 03/04/2019
    $('.esOptionCheckDetalle input').each(function() {  
        if($(this).is(':checked')){
            codigo+=$(this).attr("nuAnn")+$(this).attr("nuEmi")+$(this).attr("nuDes")+",";
            coTipoAccesoZonaNew+=$(this).attr("coservicio")+",";
            otrosDatos+=$(this).attr("coUbiDep")+$(this).attr("coUbiProv")+$(this).attr("coUbiDist")+$(this).attr("coServicio")+",";
            if(ambito.indexOf($(this).attr("ambito"))<0){
                ambito+=$(this).attr("ambito");
                coTipoAccesoZona+=$(this).attr("coservicio");//Hermes 19/09/2018
            }
        }
    });
    $('#codigo').val(codigo);
    $('#newCodigo').val(otrosDatos);
    $('#txtDetalleDestiMensAmbitos').val(ambito);
    $('#txtCoTipoAccesoZona').val(coTipoAccesoZona);//Hermes 19/09/2018
    $('#txtCoTipoAccesoZonaNew').val(coTipoAccesoZonaNew);//Hermes 03/04/2019
...
function fn_optionSelecctionDetalleMensajDestAll2(thiss){
    $('.esOptionCheck input').each(function() {          
        if($(thiss).is(':checked')){
            $(this).prop( "checked", true);
        }
...
function fu_grabarMensajeriaDocumento(){    
    console.log("fu_grabarMensajeriaDocumento");
    debugger;
    var noForm='documentoRecepMensajeriaBean';
    ...
    if(codigosDestinos != null && codigosDestinos!=""  ){
        ...
        console.log("valRetorno "+valRetorno);
        if(valRetorno==="1"){
...
function fn_rptaRegistrarDocMensajeEnvio(dataJson,noForm){ 
    console.log("fn_rptaRegistrarDocMensajeEnvio");
    
    if(dataJson!==null){ 
        console.log("dataJson.coRespuesta->"+dataJson.coRespuesta);
        if(dataJson.coRespuesta==="1"){
            var item=dataJson; 
            if(item!==null&&!!item){   
                
                console.log("item.deNro->"+item.deNro);
                console.log("item.fecha->"+item.fecha);
                console.log("item.usuario->"+item.usuario);
...
//    result += '"diasDevoluvion":"' + $('#'+noForm).find("#txtdiasDevolucion").val() + '",';     
    ...
    result += '"newCodigo":"' + $('#'+noForm).find("#newCodigo").val() + '"';  
...
function validarFormDocumentoMensaje(noForm){
   ...
   var tipoAccesoZona = $('#txtCoTipoAccesoZona').val();//Hermes 19/09/2018
   var coTipoAccesoZona = $('#txtCoTipoAccesoZonaNew').val();//Hermes 03/04/2019
   var auxCoTipoAccesoZona = coTipoAccesoZona.split(",");
   
   var vValidaNumero = ""; 
  if("NACIONAL"==deambito && (detipmsj=="MOTORIZADO" || detipmsj=="MENSAJERO")){
       valRetorno = "0"; 
        alert_Info("Aviso", "El tipo de ambito "+deambito+" NO puede enviar un "+detipmsj+".");
        $('#'+noForm).find('#deambito').focus();
  }
  else if("INTERNACIONAL"==deambito && (detipmsj=="MOTORIZADO" || detipmsj=="MENSAJERO")){
       valRetorno = "0"; 
        alert_Info("Aviso", "El tipo de ambito "+deambito+" NO puede enviar un "+detipmsj+".");
        $('#'+noForm).find('#deambito').focus();
  }
   else if(detalleambito!=deambito){
        valRetorno = "0"; 
   ...
   else if(detipenv=="URGENTE" && (detipmsj=="CORREO ELECTRONICO" || detipmsj=="MESA DE PARTES")){
       valRetorno = "0"; 
        alert_Info("Aviso", "El tipo de mensajero "+detipmsj+" NO le corresponde prioridad "+detipenv+".");
        $('#'+noForm).find('#detipenv').focus();       
   }
   else if("INTERNACIONAL"==deambito && detipenv=="URGENTE"){
       valRetorno = "0"; 
        alert_Info("Aviso", "El tipo de ambito "+deambito+" NO le corresponde prioridad "+detipenv+".");
        $('#'+noForm).find('#deambito').focus();
  }
   ...
   }else if (tipoAccesoZona=="" || !!tipoAccesoZona==false){//Hermes 19/09/2018
        valRetorno = "0"; 
        alert_Info("Aviso", "Valor en la columna tipo de zona no debe ser nulo");
   }else if (auxCoTipoAccesoZona!=""){//Hermes 03/04/2019
        if(detipenv=="URGENTE"){
            for (var i = 0; i < auxCoTipoAccesoZona.length; i++){
                var auxNewZona = auxCoTipoAccesoZona[i];
                if(auxNewZona!="01" && auxNewZona!="02" && auxNewZona!="03" && auxNewZona!="04" && auxNewZona!="08" && auxNewZona!=""){
                    valRetorno = "0"; 
                    alert_Info("Aviso", "Tipo de zona no aplica a prioridad "+detipenv+".");  
                    return
                }
            }       
        }
...
function fn_DevolverMensajeriaRecep(){ 
    var noForm='documentoEmiArBean';/*--21/08/19 HPB Devolucion Doc a Oficina--*/
    var codigosDestinos=$("#txtSelecctionOption").val(); 
    var validar=$('#txtSelecctionOptionValidar').val();
    ...
    if(validar!=""){
        if(codigosDestinos != null && codigosDestinos!=""  ){  
            if(validar.indexOf("0")>-1 || validar.indexOf("1")>-1  ){
                if(!$.trim($("#txtObservacionesDevOficina").val())){
                    bootbox.alert('Debe ingresar alguna observación para realizar la devolucion del documento.');
                }else{
                    bootbox.dialog({
                        ...
                        buttons: {
                            SI: {
                                 ...
                                 className: "btn-primary",
                                 callback: function() {  
                                 var cadenaJson=fn_buildSendJsontoServerDevMsj(noForm);/*--21/08/19 HPB Devolucion Doc a Oficina--*/
                                 ajaxCallSendJson("/srMensajeriaGestion.do?accion=goDevolverDocumentoMensajeria", cadenaJson, function(data) {/*--21/08/19 HPB Devolucion Doc a Oficina--*/
...
function fn_rptaDevolverDocMensajeEnvio(dataJson){ 
    if(dataJson!==null){ 
        if(dataJson.coRespuesta==="1"){
                //$("#btnGrabarMensajeria").prop('disabled', true);
                removeDomId('windowDevolverDocMensajeria');/*21/08/19 HPB Devolucion Doc a Oficina*/
                changeTipoBusqMensajeriaRecep('0');
                alert_Sucess("Éxito!", "Documento es recibido correctamente.");                             
        }else{
            alert_Danger("Mensajeria de Documentos: ",dataJson.deRespuesta);
...
function fn_buildSendJsontoServerDevMsj(noForm) {    
    var result = "{";
    result = result + '"obsDev":"' + jQuery('#'+noForm).find("#txtObservacionesDevOficina").val() + '",';
    result = result + '"codigosDestinos":"' + $("#txtSelecctionOption").val() + '",';
    result = result + '"feEnvOfi":"' + $("#txtSelecctionOptionFeEnvOfi").val() + '",';
    result = result + '"docEstadoMsj":"' + $('#txtSelecctionOptionValidar').val() + '"'; 
...
function fu_calcularFechaPlazo(){
    ...
    //}
    /*--------------------------Hermes 20/07/2018--------------------------*/
    var coUbiDptoDestMsj=$('#'+noForm).find("#txtcoUbiDptoDestinoMsj").val(); 
    var coUbiProvDestMsj=$('#'+noForm).find("#txtcoUbiProvDestinoMsj").val(); 
    var coUbiDistDestMsj=$('#'+noForm).find("#txtcoUbiDistDestinoMsj").val(); 
    /*---------------------------Fin 20/07/2018-----------------------------*/
    ...
    if( deNuRuc != "undefined" && deNuRuc != "-1"    ){
        if(deambito != null && deambito!=""   ){  
                ...
                if (fecenviomsj != null && fecenviomsj!="") {
                    ...
                    json += '"deTipEnv":"' + detipmsj +'",';
                    json += '"coUbiDptoDestinoMsj":"' + coUbiDptoDestMsj +'",';
                    json += '"coUbiProvDestinoMsj":"' + coUbiProvDestMsj +'",';
                    json += '"coUbiDistDestinoMsj":"' + coUbiDistDestMsj +'"';
                    json += "}";
...
function fn_optionSelecction_mensajes(){
    ...
    console.log("fn_optionSelecction2");
    var codigo="";
    var validar="";
    $('.esOptionCheck input').each(function() {  
        if($(this).is(':checked')){
...
function fn_optionSelectionMensajesAll(thiss){
    $('.esOptionCheck input').each(function() {          
        if($(thiss).is(':checked')){
            $(this).prop( "checked", true);
        }
    ...
    fn_optionSelecction_mensajes();
}

function fu_iniTblDocMensajeriaRecepNew(){
    var oTable;
    oTable = $('#myTableFixed').DataTable( {
        responsive: true,
        "searching": false      
...
function fn_DevolucionDocMensajeria() {
    var codigosDestinos=$("#txtSelecctionOption").val();
    if(codigosDestinos != null && codigosDestinos!=""  ){
        var p = new Array();
        p[0] = "accion=goDevolverDocMensajeria";
        p[1] = "pCodigosDestinos="+codigosDestinos;
...
function fn_verSeguimientoMensajeriaDevolucion(){
    var pnuAnn=$('#txtpnuAnn').val();
    var pnuEmi=$('#txtpnuEmi').val();
    var pnuSec=$('#txtpnuSec').val();
    var pnuDes = "N";
    if(!!pnuAnn&&!!pnuEmi){    
        fn_verSeguimientoObjDevolucion(pnuAnn, pnuEmi, pnuDes, pnuSec);		
    }
}
/*--21/08/19 HPB Devolucion Doc a Oficina--*/