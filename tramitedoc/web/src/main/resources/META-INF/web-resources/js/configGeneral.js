...
function fn_grabarConfPersonal() {
    ...
    var myForm={codEmp:codEmp, codDep:codDep, dirPri:dirPri, piePag:piePag, rbAcceso:rbAcceso, rbCarDoc:rbCarDoc, rbFirDoc:rbFirDoc, rbDocDef:rbDocDef};
...
function fn_cargarBusqProveedores() {
    ...
    if(!!busRazonSocial){
        ...
        if(allTrim(busRazonSocial).length >= 0 && allTrim(busRazonSocial).length <= 2){  //El texto es entre 1 y 3 caracteres
...
function fu_goEditarProveedor() {
    ...
    if (!!codProveedor) {
        ...
        alert_Warning("ATENCIÓN", "seleccionar una FILA");
    }
}
...
function fn_cargarBusqOtrosOrigenes() {
    ...
    if(!!deRazSocOtr){
        ...
        if( allTrim(deRazSocOtr).length >= 0 && allTrim(deRazSocOtr).length <= 3 ){
...
//    });
     ...
     * Cierra el formulario (modal) para realizar búsqueda de dependencia para el empleado
     */
    $(document).on('click','#windowConsultaLocal #btn_salir',function(){
        removeDomId('windowConsultaLocal');
    });
    ...
    $(document).on('click','#formEditPersonal #user_add',function(){
        ...
        pat = pat.replace(/[^a-zA-Z]/g,'');
        pat = (pat.length>20?pat.substring(0, 19):pat);
        var item = '<td style="width: 30%">Nombre usuario</td>'
            +'<td style="width:70%">'
        ...
        var itemPerfil = ' <td style="width:50%;vertical-align:top;">'
                        +'    <fieldset>'
                        ...
                        +'                    <td>Menú:</td>'
                        +'                    <td>'
                        +'                        <select id="codPerfil"  style="width:108px;" onchange="" >'
                        +'                            <option value="1" selected="selected">BASICO</option>'
...
function valFormOtroOrigenConfigGeneral(){
    ...
    if(valRetorno==='1'){
        if(!!pRazonSocial&&allTrim(pRazonSocial).length>1){
            ...
            valRetorno = '0';
            bootbox.alert("<h5>Ingrese razón social campo obligatorio.</h5>", function() {
                bootbox.hideAll();
                jQuery("#txtRazonSocial").focus();
            });            
        }
...
function valFormDependenciaConfigGeneral(noForm){
    ...
    if(valRetorno==="1"){
        /*[HPB] Inicio 21/04/23 CLS-087-2022 */
        //if(!!pdeSigla&&allTrim(pdeSigla).length>1){
        if(!!pdeSigla&&allTrim(pdeSigla).length>0){
        /*[HPB] Fin 21/04/23 CLS-087-2022 */    
            ...
            bootbox.alert("<h5>Ingrese Sigla de Dependencia campo obligatorio, mínimo 1 caracter</h5>", function () {
            /*[HPB] Fin 21/04/23 CLS-087-2022 */
                bootbox.hideAll();
                jQuery(noForm).find("#deSigla").focus();
            });
    ...
    if(valRetorno==="1"){
      ...
      if(!!pdeCortaDepen&&allTrim(pdeCortaDepen).length>0){
      /*[HPB] Fin 21/04/23 CLS-087-2022 */
        ...
        if(pdeCortaDepen.length+nrolinespDeCortaDep > maxCarateres){
            ...
            bootbox.alert("<h5>Ingrese Sigla Corta campo obligatorio, mínimo 1 caracter.</h5>", function () {
            /*[HPB] Fin 21/04/23 CLS-087-2022 */
                bootbox.hideAll();
                jQuery(noForm).find("#deCortaDepen").focus();
            });
...
function fn_iniRegistroCiudadanoNuevo(vnuLem){
    ...
    $('.deDomicil').bind('copy paste', function (e) {
           e.preventDefault();
    });
...
function fn_grabarNuevoRequisito() {
    ...
    if (allTrim(DeDescripcion).length<1) {
        bootbox.alert("<h5>Ingrese descripción del Rquisito, campo obligatorio.</h5>", function() {
            bootbox.hideAll();
            jQuery("#txtDeNombre").focus();
        });           
    } else {
...
function fn_getProveedorDesdeRuc(){
    var vReturn=0;
    var url = "/srTablaConfiguracion.do?accion=goGetProveedorDesdeRuc";
    if($('#txtRuc').val().length===11&&fu_esNumerico($('#txtRuc').val())==='SI'){
        ajaxCall(url,{ruc: $('#txtRuc').val()}, function(data) {
            if(data.exito){
                if(!data.si_existe && data.disponible){
                    $('#txtRazonSocial').val(data.descripcion.trim());
                    $('#txtUbiDomicilio').val(data.descripcionUbigeo.trim());
                    $('#txtDireccion').val(data.deDireccion.trim());                    
                    $('#hidN_Ruc').val(data.nuRuc.trim());
                    $('#hCodDepCon').val(data.idDepartamento.trim());
                    $('#hCodProPa').val(data.idProvincia.trim());
                    $('#hCodDisDep').val(data.idDistrito.trim());                     
                    vReturn=1;
                } else {
                    fn_cleanPersonaDesdeDni();
                    alert_Warning("ATENCIÓN",data.mensaje);
...
/*--HPB 13/01/20 Integrar PIDE--*/
/*-- [HPB] Inicio 26/09/22 OS-0000768-2022 --*/
function fn_cargarAccesosAdicionales() {
    var p = new Array();
    p[0] = "accion=goCargarAccesoNuevo";
    p[1] = "vCoDependencia=" + jQuery('#pCodEmp').val();
    p[2] = "vUserName=" + jQuery('#username_tmp').val();
...
function fn_buscaDependenciaAccesoAdicional(cell) {
    var p = new Array();
    p[0] = "accion=goBuscaDependenciaAccesoAdicional";
    p[1] = "pdeDepen=" + fu_getValorUpperCase(($(cell).parent()).find('input[type=text]').val());
    ajaxCall("/srTablaConfiguracion.do", p.join("&"), function(data) {
        fn_rptaBuscaDependenciaAccesoAdicional(data);
        jQuery('#txtTblAccesoAdiFilaWhereButton').val((($(cell).parent()).parent()).index());
        jQuery('#txtTblAccesoAdiColWhereButton').val(($(cell).parent()).index());
    },
            'text', false, false, "POST");
}

function fn_rptaBuscaDependenciaAccesoAdicional(XML_AJAX) {
    if (XML_AJAX !== null) {
        $("body").append(XML_AJAX);
    }
}

function fn_eventAccesoAdicionalDet() {
    fn_eventTblAccesoAdicionalGuardaIndex("tblAccesosAdicionales", "txtIndexFilaAccesoAdicional");
}

function fn_eventTblAccesoAdicionalGuardaIndex(idTabla, idIndexFilaSelec) {
    var indexFilaClick = -1;
...
function fn_removeDependenciaAcceso() {
    fn_removeDependenciaAccesoAdicional("1", "tblAccesosAdicionales", 'txtIndexFilaAccesoAdicional');
}

function fn_addDependenciaAcceso() {
    fn_addDependenciaAccesoAdicional('tblAccesosAdicionales');
}

function fn_addDependenciaAccesoAdicional(idTabla) {
    if ($('#' + idTabla + ' >tbody >tr').length === 1 || $("#" + idTabla + " tbody tr td").last().html() === "1") {
        var found = true;
        $("#" + idTabla + " tbody tr").each(function(index, row) {
            if (index > 0) {
        ...
        if (found) {
           ...
           bootbox.alert("Corregir Dependencia");
        }
    } else {
       bootbox.alert("Corregir Dependencia");
    }
...
function fn_removeDependenciaAccesoAdicional(indexAccionBD, idTabla, indexFilaRemove) {
    ...
    if (pindexFilaDesEmiDoc !== "-1") {
        ...
        if ($("#" + idTabla + " tbody tr:eq(" + pindexFilaDesEmiDoc + ") td").last().html() === "1") {
            bootbox.dialog({
                message: " <h5>¿ Esta Seguro de Quitar Dependencia ?</h5>",
                buttons: {
                    SI: {
                        label: "SI",
                        className: "btn-primary",
       ...
       bootbox.alert("Seleccione Dependencia.");
    }
}

function fu_setDatoDependenciaAccesoAdicional(cod, desc) {
    destinatarioDuplicado = true;
    var pfila = jQuery('#txtTblAccesoAdiFilaWhereButton').val();
    var pcol = jQuery('#txtTblAccesoAdiColWhereButton').val();
    var idTabla = "tblAccesosAdicionales";
    var arrCampo = new Array();

    arrCampo[0] = (pcol * 1 + 2) + "=" + cod;//codDependencia
    var bResult = fn_validaDependenciaAdicionalDuplicado('tblAccesosAdicionales', arrCampo, true, pfila*1);
    
    if (bResult) {
        $("#" + idTabla + " tbody tr:eq(" + pfila + ") td:eq(" + pcol + ")").find('input[type=text]').val(desc);
        $("#" + idTabla + " tbody tr:eq(" + pfila + ") td:eq(" + (pcol * 1 + 2) + ")").text(cod);
        removeDomId('windowconsultaDependenciaAccesoAdicional');
        ...
        ajaxCall("/srDocumentoAdmEmision.do", p.join("&"), function(data) {
            if (data !== null && allTrim(data.coRespuesta) === "1") {
                ...
                if (bResult) {
                    ...
                    if(bDestinOk){
                        fn_changeDestinatarioCorrectoAccesoAdicional(pfila);
                    }
                }                    
            } else {
        ...
        destinatarioDuplicado=true;
        removeDomId('windowconsultaDependenciaAccesoAdicional');
       bootbox.alert("Dependencia ya se encuentra en la lista.");
    }
}

function fn_changeDestinatarioCorrectoAccesoAdicional(pnroFila) {
    var ultimDiv = $("#tblAccesosAdicionales tbody tr:eq(" + pnroFila + ") td div.ui-state-error-text").last();
    ultimDiv.removeClass('ui-state-error-text');
    var ultimDivSpan = ultimDiv.find('span');
    ultimDivSpan.removeClass('ui-icon-alert');
    ultimDivSpan.addClass('ui-icon-check');
    ultimDivSpan.attr("title", "OK");
    ultimDiv.addClass('ui-state-highlight-text');
    $("#tblAccesosAdicionales tbody tr:eq(" + pnroFila + ") td").last().html("1");
}

function fn_grabarDependenciaAdicional(){
    var idTblDepAdi='#tblAccesosAdicionales';
    var msg='Corregir Dependencia';
    
    if(fn_verificarLstDepAdiCorrecto(idTblDepAdi,msg)){
        var objTrxJson=fn_getObjTrxJsonDepenAdicional();
        var rpta=fn_verificarChangeDepenAdic(new Function('return '+objTrxJson)());        
        var nrpta=rpta.substr(0, 1);
        if(nrpta === "1"){    
            var url = "/srTablaConfiguracion.do?accion=goNuevoDepAdic";
            ajaxCallSendJson(url, objTrxJson, function(data) {
                if (data === "NO_OK") {
                    alert_Danger("ERROR:", "Error al insertar dependencia adicional.");
                }else if (data === "DEL") {
                    alert_Danger("ERROR:", "No se puede quitar la dependencia a la que esta asignado.");
                /*-- [HPB] Inicio 16/01/23 CLS-087-2022 --*/
                }else if (data === "UPD") {
                    alert_Danger("ERROR:", "No se puede retirar del listado de trabajadores de la dependencia a la que esta asignado.");
                /*-- [HPB] Fin 16/01/23 CLS-087-2022 --*/
                }else {
                    fn_actualizarTblDepAdic(idTblDepAdi);
                    alert_Sucess("Dependencia :", "Datos Guardados.");
                }
            }, 'text', false, false, "POST");
        }else{
...
function fn_delFilaTblDepenAdicional(idTblEmp){
   jQuery(idTblEmp+' tbody tr').each(function (index){
       if(index>0){
           var ptr=jQuery(this);
           if(ptr.hasClass('oculto')||ptr.find('td').eq('1').text()==="DEL"){
...
function fn_verificarChangeDepenAdic(objTrxDependencia){
    var vReturn = "0LA UNIDAD ORGANICA ES LA MISMA.";    
    if (!!objTrxDependencia) {
        if (!!objTrxDependencia.empleadoDependenciaDetalle) {
            if (objTrxDependencia.empleadoDependenciaDetalle.length > 0) {
                vReturn = "1A GRABAR";
            }
        }
    }
...
function fn_getObjTrxJsonDepenAdicional(){
    var result="{";
    result = result + '"coUsuario":"' + $("#username_tmp").val() + '",';
    result = result + '"coDependencia":"' + $("#coDependencia").val() + '",';
    result = result + '"cempCodemp":"' + $("#codEmp").val() + '",';
    result= result + '"empleadoDependenciaDetalle":' + sortDelFirst(fn_tblDepenAdicionalToJson());
    return result + "}";
}

function fn_tblDepenAdicionalToJson() {
    var arrColMostrar = new Array();
    arrColMostrar[0] = "accionBD=2";
    arrColMostrar[1] = "coDependencia=3";
    arrColMostrar[2] = "tiAcceso=4";
    arrColMostrar[3] = "inConsulta=5";
    arrColMostrar[4] = "tiAccesoMp=6";
    arrColMostrar[5] = "inConsultaMp=7";
    /*-- [HPB] Inicio 16/01/23 CLS-087-2022 --*/
    arrColMostrar[6] = "inTrabajador=8";
    //return '['+fn_tblDestEmihtml2json('tblAccesosAdicionales', 1, arrColMostrar, 8, "1", 2, "BD")+']';
    return '['+fn_tblDestEmihtml2json('tblAccesosAdicionales', 1, arrColMostrar, 9, "1", 2, "BD")+']';
    /*-- [HPB] Fin 16/01/23 CLS-087-2022 --*/
}

function fn_verificarLstDepAdiCorrecto(idTabla,msg) {
    if ($(idTabla + ' >tbody >tr').length === 1 || $(idTabla + " tbody tr td").last().html() === "1" || $(idTabla + ' >tbody >tr').length === 0) {
        var found = true;
        $(idTabla + " tbody tr").each(function(index, row) {
            if (index > 0) {
...
function changeAccionBDDepAdic(cmbChange) {
    if (($(cmbChange).parent()).parent().find("td").eq(2).html() === "BD") {
        ($(cmbChange).parent()).parent().find("td").eq(2).html("UPD");
    }
    
    if (jQuery(cmbChange).parents(':eq(1)').children().get(1).innerHTML==="BD") {
       jQuery(cmbChange).parents(':eq(1)').children().get(1).innerHTML="UPD";
    }
}

function fn_validaDependenciaAdicionalDuplicado(idTabla, arrCamposCompara, bcomFilaActual, indexFilaActual) {
    var countFound = 0;
        ...
        if (index > 0 && !$(row).hasClass('oculto')) {
            if (bcomFilaActual) {
                if (index !== indexFilaActual) {
                    if ($(row).find("td:eq(" + indexColDependencia + ")").text() === coDepen) {
                        countFound++;
                    }
                }
            } else {
...
function fn_actualizarTblDepAdic(idTblEmp){
   jQuery(idTblEmp+' tbody tr').each(function (index){
       if(index>0){
           var ptr=jQuery(this);
           if(ptr.hasClass('oculto')){
...
/*-- [HPB] Fin 26/09/22 OS-0000768-2022 --*/
/*-- [HPB] Inicio 23/02/23 CLS-087-2022 --*/
function fu_goAdmEmpleadoLocal() {
    ajaxCall("/srTablaConfiguracion.do?accion=goAdmEmpleadoLocal",{"criterio":$('#deLocal').val()}, function(data) {
        $("body").append(data);
    }, 'text', false, false, "POST");
}
/*-- [HPB] Fin 23/02/23 CLS-087-2022 --*/